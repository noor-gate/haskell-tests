...
getAttribute s (Element _ x _)
  = fromMaybe "" $ lookup s x
getAttribute _ _
  = ""

getChildren :: String -> XML -> [XML]
getChildren s m@(Element a _ x)
  | s == a = m : conc
  | otherwise = conc
    where
      conc = concatMap (getChildren s) x
getChildren _ _
  = []

getChild :: String -> XML -> XML
getChild s x
  | null $ getChildren s x = Text ""
  | otherwise = head $ getChildren s x

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild s (Element n a x)
  = Element n a (x ++ [s])

getValue :: XML -> XML
getValue (Element _ _ x)
  = Text (concatMap (fromText) (map getValue x))
    where
      fromText (Text a) = a
getValue (Text t)
  = Text t
...
parseAttributes []
  = ([], "")
parseAttributes str@(t:ts)
  | t == '>' = ([], ts)
  | otherwise = ((attN, attV) : recA, recB)
    where
      (attN, p) = parseName $ skipSpace str
      (attV, r) = break (=='"') (tail $ dropWhile (/='"') p)
      (recA, recB) = parseAttributes $ skipSpace (tail r)
...
parse' "" (Element _ _ es : _)
  = head es
parse' s@(c : c' : cs) e
  | c == '<' && c' == '/' = parse' remA $ popAndAdd e
  | c == '<' && c' /= '/' = parse' remA ((Element name parsedAtt []) : e)
  | otherwise = parse' remB (addText (takeWhile (/='<') s) e)
    where
      remA = (tail $ dropWhile (/='>') s)
      remB = (dropWhile (/='<') s)
      (name, att) = parseName $ tail $ takeWhile (/='>') s
      parsedAtt = fst (parseAttributes att)
